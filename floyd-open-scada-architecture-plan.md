# Floyd BioEnergy Lightweight Open-Source SCADA Plan

Status: review draft  
Date: 2026-05-25  
Intent: architecture and implementation plan only. No implementation should start until Floyd reviews the decisions and open questions below.

Roadmap note: treat this as the Complete Version target. Implement the simplified MVP first using [floyd-scada-mvp-to-complete-roadmap.md](/Users/kopfmann/Documents/scada/floyd-scada-mvp-to-complete-roadmap.md).

## 1. Read-In Summary

Inputs reviewed:

- `/Users/kopfmann/Library/CloudStorage/OneDrive-Personal/Elinext/Floyd Bioenergy/floyd-architecture-detailed.md`
- `/Users/kopfmann/Library/CloudStorage/OneDrive-Personal/Elinext/Floyd Bioenergy/floyd-packages.mmd`
- `/Users/kopfmann/Library/CloudStorage/OneDrive-Personal/Elinext/Floyd Bioenergy/floyd-deployment.mmd`
- `/Users/kopfmann/Library/CloudStorage/OneDrive-Personal/Elinext/Floyd Bioenergy/floyd-bioenergy-scada-MVP-backlog-v2.xlsx`
...
Build a lightweight, open-source-first SCADA platform by composing proven infrastructure and writing only the Floyd-specific pieces: edge acquisition, Sparkplug ingestion, asset/tag model, alarm logic, operator UI, reports, and one PdM baseline.

Recommended field-side choice for MVP: **Option B, IPC/gateway-based edge agent**.

Rationale:

- The backlog already assumes an F60 IPC edge agent, S7/Modbus drivers, local buffering, Sparkplug B publishing, and per-unit identity.
...
| Frontend | React + Vite static SPA | Smaller than SSR for this use case; can still move to Next.js if Floyd prefers the original document recommendation. |
| Realtime UI | WebSocket from API | Push live tag values and alarms to the browser. |
| Auth | App-local users + one OIDC provider | Matches MVP backlog. Keycloak optional only if Floyd wants self-hosted IAM instead of direct OIDC integration. |
...
Decision point: if Floyd accepts source-available infrastructure, EMQX can replace Mosquitto for richer Sparkplug processing. If strict OSI-style open source matters, use Mosquitto and keep Sparkplug state handling in our ingestion service.
...
- Floyd support access is cross-tenant but audited.
...
- Site view for one or more F60 units plus optional BESS / material-handling equipment.
- F60 unit overview with engine, gasifier, gas-cleaning, heat-recovery, biochar, and safety groups.
...
- Produce canonical F60 tag map and safety tag list.

Exit gate:

- Floyd approves Option B edge architecture or explicitly chooses PLC-direct fallback.
...
- OPC UA/Modbus TCP driver path, with Siemens S7 implemented only if the F60 controller actually requires it.
...
- One F60-like unit or simulator streams 500 tags at 1Hz through outage/reconnect.
...
- F60 asset template and tag tree instantiation.
- Tag current-state API and tag browser API.

Demo:

- Floyd tenant and one customer tenant see isolated units and tags.
...
- Fleet view, F60 overview, trend explorer, tag browser.
...
- Operator can monitor live F60 status, inspect trends, and use Japanese UI on demo screens.
...
2. Is Option B edge hardware already present in the F60 cabinet, or must it be added?
3. Which controller/protocol is actually exposed by the F60: OPC UA, Modbus TCP, Siemens S7, Beckhoff ADS/TwinCAT, Schneider/Modicon, or something else?
...
## 13. F60 Research Addendum

This addendum was added after reviewing the local F60 data sheet and researching Floyd plus comparable biomass CHP / biogas operators online.

### What The F60 Is Like

The F60 should be treated as a compact industrial wood-gas CHP asset, not merely a generic telemetry device.
...
Public Floyd positioning adds more product context:

- Floyd presents F60 as continuous, modular, 24/7 energy production for industrial/agricultural sites with dry biomass waste.
...
- If Floyd expands into bioreactors/biogas later, that should be a separate asset template. It should not distort the F60 MVP unless a bioreactor customer is actually in scope.

### Plan Adjustments Required

1. Change the product model from `unit-first` to `site-first`.

   A real customer site may have multiple F60 units, a BESS, conveyors, moving floors, wood-chip storage, dryer/pre-dryer, hot-water loop, grid connection, and optional biochar handling. The plan now includes `sites` as a first-class entity.

2. Promote fuel quality and fuel inventory into MVP or near-MVP.

   The original plan only had tags/historian. F60 performance depends heavily on wood-chip moisture and quality. Add fuel lots, moisture checks, supplier/source, delivery notes, storage level, and fuel-consumption KPIs.

3. Add biochar lot tracking and MRV readiness.

   A monthly energy report is too narrow if Floyd sells biochar/carbon-credit upside. Add biochar lots, production volume/mass, disposal/use destination, feedstock linkage, and exportable evidence. Full carbon-credit workflow can wait, but the data model should not be retrofitted later.
...
5. Expand the F60 template.

   Replace the generic F60 groups with: fuel receiving/storage, dryer/pre-dryer, conveyor/feed system, gasifier, gas filtering/cleaning, flare/start-stop handling, engine/genset, heat recovery/hot-water loop, exhaust/after-treatment, biochar discharge, compressed air, ventilation/ambient, safety, BESS optional, and grid interconnect.

6. Revisit the driver priority.

   Do not commit to Siemens S7 as the first driver until Floyd confirms the controller. For a lightweight open-source SCADA, OPC UA + Modbus TCP should be the default integration path, with S7, Beckhoff ADS/TwinCAT, or Schneider-specific drivers added if the actual F60 control stack requires them.
...
10. Keep bioreactor support out of F60 MVP unless explicitly needed.

   If "bioreactor" means anaerobic digestion/biogas, create a later `Biogas_AD` asset template with biological process monitoring. Do not mix pH/VFA/alkalinity logic into the F60 wood-gas CHP template.
