# Floyd F60 Lightweight Open-Source SCADA: Simplified Plan

Status: simplified review draft  
Date: 2026-05-25  
Purpose: reduce the previous architecture to the smallest useful SCADA MVP for one Floyd F60 demo unit, while leaving a path to grow later.

Roadmap link: use this as Stage 1 in [floyd-scada-mvp-to-complete-roadmap.md](/Users/kopfmann/Documents/scada/floyd-scada-mvp-to-complete-roadmap.md).
...
- One customer, one F60 unit, one region.
...
Anything not needed to prove the F60 demo is deferred.

## 2. Simple Architecture

```mermaid
flowchart LR
    PLC["F60 PLC / controller"] -->|"OPC UA or Modbus TCP<br/>S7 only if confirmed"| EDGE["Edge gateway<br/>Node-RED first"]
...
| PLC protocols | **OPC UA or Modbus TCP first** | Do not assume Siemens S7 until Floyd confirms it. |
...
```text
tenant.site_name = "Customer A - Factory 1"
```
...
## 5. F60 Subsystems For MVP

Model only the subsystems visible in the F60 data sheet and useful for demo:
...
### Phase 0: Confirm F60 Interface, 2-3 days
...
- We can read live or simulated F60 tags.
...
- Backend service.
- Basic web HMI shell.
- Simulated F60 publisher.

Demo:

- Fake F60 data appears live in the browser.
...
- Real F60 tags trend in the HMI.
...
- Operator can monitor the F60 and receive alarms.
...
That is enough to prove the value: live F60 monitoring, alarms, trends, and a monthly report.
