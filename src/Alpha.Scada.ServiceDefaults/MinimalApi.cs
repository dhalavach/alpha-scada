...
public static class MinimalApi
{
    ...
    public static async Task<IResult> MetricsAsync(string serviceName, NpgsqlDataSource dataSource, CancellationToken cancellationToken)
    {
        ...
        metrics.AppendLine("# HELP alpha_scada_wolverine_outbox_depth Pending Wolverine outgoing envelopes by service");
        metrics.AppendLine("# TYPE alpha_scada_wolverine_outbox_depth gauge");
        metrics.AppendLine(CultureInfo.InvariantCulture, $"alpha_scada_wolverine_outbox_depth{{service=\"{serviceLabel}\"}} {outboxDepth}");
