...
app.MapGet("/internal/v1/reports/monthly", async (HttpContext context, ReportingRepository repository) =>
{
...
app.MapPost("/internal/v1/reports/monthly/run", async (ReportRunRequest request, HttpContext context, ReportingService service) =>
{
    var user = HttpUserContext.FromHeaders(context.Request.Headers);
    if (user is null) return Results.Unauthorized();
    return Results.Ok(await service.RunMonthlyAsync(request, user, context.RequestAborted));
});
